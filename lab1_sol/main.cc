...
Student* findStudentById(const std::vector<Student*>& studentVec, const std::string& id){
...
void AppX::loadFiles()
{
    ...
    while(getline(clfile, line)){
        ...
        if (line.find("Class:") == 0) {
            ...
            point = stoi(line.substr(7));
            cl = new Class(clsname, point);
            classVec.push_back(cl);
        } else {
            st = findStudentById(studentVec, line);
            if (st != nullptr && cl != nullptr) {
                cl->addStudent(*st);
                st->addClasses(cl);
            }
        }
...
void AppX::inputScore()
{
    ...
    while (true) {
        ...
        Student *st = findStudentById(studentVec, sbuf);
        if (st == nullptr) {
        ...
        try {
            cl->getStudentWrapper(st->id).setScore(score);
        } catch (const char *e) {
            cerr << e << endl;
        } catch (string e) {
            cerr << e << endl;
